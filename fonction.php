...
function create_user($login, $password)
{
    ...
    file_put_contents($filePath, $motdepassecrypter);
    return 0;
}
...
function connect_user($login, $password)
{
    ...
    $motdepassestockage = file_get_contents($filePath);
    if (!password_verify($password, $motdepassestockage)) {
        return 1;
    }
...
function Passwordverify($password)
{
    ...
    $preg = '/[' . preg_quote($allowed_special_chars, '/') . ']/';
